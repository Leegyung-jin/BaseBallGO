...
public class DinosServiceImpl implements DinosService {
    ...
    // List
    @Override
    public PageResultDTO<StadiumDTO, DinosStadium> getList(PageRequestDTO requestDTO) {
        Pageable pageable = requestDTO.getPageable(Sort.by("sno").descending());

        // 검색
        BooleanBuilder booleanBuilder = getSearch(requestDTO);
        Page<DinosStadium> result = repository.findAll(booleanBuilder, pageable);  // Querydsl 사용

        Function<DinosStadium, StadiumDTO> fn = (entity -> entityToDTO(entity));
    ...
    private BooleanBuilder getSearch(PageRequestDTO requestDTO) {
        ...
        QDinosStadium qDinosStadium = QDinosStadium.dinosStadium;

        // sno > 0조건 생성
        BooleanExpression expression = qDinosStadium.sno.gt(0L);
    ...
    public StadiumDTO getStadium(Long sno) {
        ...
        DinosStadium stadium = (DinosStadium) result.get(0)[0];               // Movie 엔티티는 가장 앞에 존재한다. - 모든 Row가 동일한 값
        List<DinosStadiumImage> stadiumImageList = new ArrayList<>();    // 영화의 이미지 개수만큼 MovieImage가 필요하다.

        result.forEach(arr -> {
            DinosStadiumImage stadiumImage = (DinosStadiumImage) arr[1];
